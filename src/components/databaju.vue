...
async function fetchBajuData() {
  try {
    const response = await fetch('https://tokobajudela.glitch.me/baju')
...
async function updateBaju() {
  if (editIndex.value !== null) {
    try {
      const response = await fetch(`https://tokobajudela.glitch.me/baju/${editIndex.value}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json'
        },
...
async function hapusBaju(id) {
  if (confirm('Apakah Anda yakin ingin menghapus baju ini?')) {
    try {
      const response = await fetch(`https://tokobajudela.glitch.me/baju/${id}`, {
        method: 'DELETE'
      })
